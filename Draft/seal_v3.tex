...
To reduce the network latency WSPs need to allocate services location where has the lower latency the user center that
...
integer scalarization technique \cite{Multiobjective}, 
\begin{enumerate}
...
So far, to the best of our knowledge, there is no research has considered the service location-allocation problem as a multi-objective problem. Therefore, this paper we will presents a NSGA-II-based approach to the service location allocation
problem.

Since the nature of service location-allocation problem is a multi-objective problem. Therefore, an evolutionary multi-objective optimization (EMO) methodologies is considered.
...
In addition to deciding locations to of the services, information about network latency between user centers and candidate locations. 
Exist datasets in \cite{6076756} \cite{5552800} are latency information collected from the real world. 
...
\begin{itemize}
	...
	\item To evaluate our proposed approaches using some existing datasets.
...
The experimental design and results evaluation are shown in Section \ref{sec:experiment}.
...
\begin{equation}
		Fitness = weight \times CostFitness + (1 - weight) \times LatencyFitness
	\end{equation}
...
good condition (70\%), pool condition (40\%) and minimum budget condition (0\%). We try to stimulated the real budget condition with these four 
...
To compare with the result of NSGA-II and GA, we derive the Pareto front by using Xue's approach \cite{Xue} \cite{6381531}, then take the approach from \cite{1688438} to compare the results.
...
In addition to the comparison between NSGA-II and GA, we conducted full experiments on NSGA-II with initialisation and GA with initialisation.
We initialized the population with 49 random chromosomes and one minimum cost chromosome. We expect the initialized algorithms superior than
...
Algorithms run under four sizes of matrices: 3 $\times$ 3, 5 $\times$ 5, 10 $\times$ 10 and 15 $\times$ 15 with different levels of cost constraints: sufficient, good, poor and minimum.
...
In 3 $\times$ 3 and 5 $\times$ 5 dataset, the solutions from NSGA-II and initialized NSGA-II are mostly overlapping. The solutions from GA and
initialized GA are also interleaving. That is because of size of matrices are small, it is easy for algorithms to find near optimal solutions. 
Therefore, it is hard to claim initialized algorithms are superior than uninitialized algorithms, but clearly, NSGA-II is dominate GA.
...
Sufficient 		&4.4 $\pm$ 0.3		& 1.6	$\pm$ 0.1	& 5.9 $\pm$ 0.1& 3.2 $\pm$ 0.1	& 13.7 $\pm$ 0.1&11.0 $\pm$ 0.1 & 27.0 $\pm$ 0.1 &23.9 $\pm$0.5\\ \hline
 Good 			&4.4 $\pm$ 0.2		& 1.6 $\pm$0.1      &6.0 $\pm$ 0.1 &3.2 $\pm$ 0.1  &13.9 $\pm$ 0.08  & 11.1 $\pm$ 0.3  &27.2 $\pm$ 0.1          &24.1 $\pm$ 0.27      \\ \hline
 Poor 			&4.6 $\pm$ 0.19	& 2.2 $\pm$0.2          &6.3 $\pm$ 0.07    &4.29 $\pm$ 0.17          &15.2 $\pm$ 0.16          & 14.8 $\pm$ 0.3          &31.3 $\pm$ 0.28& 33.6 $\pm$ 0.45          \\ \hline
 Minimum 		&4.6 $\pm$ 0.1		& 2.2 $\pm$ 0.1   &7.2 $\pm$ 0.12   &5.75 $\pm$0.17          &24.12 $\pm$ 0.5          & 25.8 $\pm$0.5          &56.72 $\pm$ 1.6           &  66.8$\pm$1.2         \\ \hline
\end{tabular}
\end{table}
